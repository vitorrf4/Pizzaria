...
assessment.alertSaveStatus = function( status, forSaveAll, maxInputSize = -1 )
{
  if ( status == 'COMPLETED' )
  {
  ...
  else if ( status == 'ERROR_ANSWER_TOO_LONG' )
  {
    if ( !forSaveAll )
    {
      alert( page.bundle.getString( "confirm.error.answer_too_long", maxInputSize ) );
    } else
    {
      alert( page.bundle.getString( "confirm.error_all" ) );
    }
  }
  else
  {
...
assessment.getQuestionItemResponseHelper = function( originalRequest, interactive )
{
  ...
  if ( interactive )
  {
    if ( status == "ERROR_ANSWER_TOO_LONG" )
    {
      var maxInputSize = assessment.getAdditionalResponseDataValue( originalRequest, "MaxInputSize" );
      assessment.alertSaveStatus( status, false, maxInputSize );
    } else
    {
...
assessment.AIMcompleteCallbackHelper = function( response, interactive, forSaveAll )
{
  ...
  try
  {
    ...
    assessment.setQuestionStatus( status == "COMPLETED", status != "ERROR" && status != "ERROR_ANSWER_TOO_LONG");
...
assessment.parseAjaxResponseEx = function( response )
{
  ...
  if ( typeof ( response ) != 'undefined' && response !== '' )
  {
    ...
    if ( parts.length > 1 )
    {
      ...
      if ( status != 'COMPLETED' && status != 'UNANSWERED' && status != 'PARTIAL' && status != 'ERROR_ANSWER_TOO_LONG' )
      {
        status = 'ERROR';
      }
      if ( elapsed != -1 )
...
assessment.getAdditionalResponseDataValue = function( originalRequest, valueToGet )
{
  var response = originalRequest.responseText;
  var value = '';
  if ( typeof ( response ) == 'undefined' || response === '' || !response.includes( "Additional_response_data:" ) )
  {
    return value;
  }

  // Getting rid of status and elapsedTime values.
  var [ status, elapsedTime, ...additionalResponseData ] = response.split( ',' );
  additionalResponseData = additionalResponseData.join( ',' );
  // Cleaning and parsing additional response data.
  additionalResponseData = additionalResponseData.replace( '{', '' );
  additionalResponseData = additionalResponseData.replace( '}', '' );
  additionalResponseData = additionalResponseData.split( ':' )[ 1 ];
  // Deconstructing a java HashMap.toString output.
  var additionalResponseDataList = additionalResponseData.split( ',' );
  value = additionalResponseDataList.filter( myValue => myValue.match( valueToGet ) )[ 0 ];
  value = value.split( '=' )[ 1 ]
  return value;
}
